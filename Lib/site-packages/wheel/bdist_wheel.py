...
def get_flag(var, fallback, expected=True, warn=True):
    ...
    if val is None:
        if warn:
            warnings.warn(
                "Config variable '{0}' is unset, Python ABI tag may " "be incorrect".format(var),
                RuntimeWarning,
                2,
            )
        return fallback
...
def get_abi_tag():
    ...
    if not soabi and impl in ("cp", "pp") and hasattr(sys, "maxunicode"):
        ...
        if get_flag("Py_DEBUG", hasattr(sys, "gettotalrefcount"), warn=(impl == "cp")):
            d = "d"
        if get_flag(
            "WITH_PYMALLOC", impl == "cp", warn=(impl == "cp" and sys.version_info < (3, 8))
...
class bdist_wheel(Command):
    ...
    def run(self):
        ...
        setattr(
            install, "install_purelib" if self.root_is_pure else "install_platlib", basedir_observed
        )
        ...
        distinfo_dirname = "{}-{}.dist-info".format(
            safer_name(self.distribution.get_name()), safer_version(self.distribution.get_version())
        )
        ...
        getattr(self.distribution, "dist_files", []).append(
            ("bdist_wheel", "{}.{}".format(*sys.version_info[:2]), wheel_path)  # like 3.7
        )
    ...
    def license_paths(self):
        ...
        files = set()
        patterns = sorted({option for option in metadata.get("license_files", ("", ""))[1].split()})
    ...
    def egg2dist(self, egginfo_path, distinfo_path):
        ...
        if os.path.isfile(egginfo_path):
            ...
            shutil.copytree(
                egginfo_path,
                distinfo_path,
                ignore=lambda x, y: {"PKG-INFO", "requires.txt", "SOURCES.txt", "not-zip-safe"},
            )
